...
namespace GS.Land.Receivers
{
    ...
    public class IssueItem : SPItemEventReceiver
    {
        ...
        private static readonly string issueNumberFieldName = "IssueNumberTextLand";
        ...
        {
            using (new SPMonitoredScope("GS.Land.IssueItemAdded"))
            {
                    ...
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        using (var site = new SPSite(properties.SiteId))
                        {
                            using (var web = site.OpenWeb(properties.RelativeWebUrl))
                            {
                                ...
                                DocumentSet.Create(folder, qTitle, tLib.ContentTypes["IssueAttachmentLand"].Id, props,
                                    true);
                            }
                        }
                    });
        ...
        protected void RenameAttachment(SPItemEventProperties properties)
        {
            try
            {
                ...
                using (new SPMonitoredScope("GS.Land.IssueItemUpdated"))
                {
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        using (var site = new SPSite(properties.SiteId))
        ...
        protected void DeleteAttachment(SPItemEventProperties properties)
        {
            ...
            using (new SPMonitoredScope("GS.Land.IssueItemDeleting"))
            {
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    using (var site = new SPSite(properties.SiteId))
