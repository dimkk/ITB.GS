...
(function () {
    ...
    function renderFields(context) {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.DisplayForm) {
            ...
            if (a) {
                $(a).attr("id", "ReportAssignmentZkh");
                assignmentLinkHtml = htmldoc.html();
            }
        }
        resultHtml += assignmentLinkHtml;

        resultHtml += renderFieldBlock('Дата', 2, 4, "ReportDateZkh");
        resultHtml += '</div>';
        ...
        resultHtml += '<div class="form-group">';
        resultHtml += renderFieldBlock('Текст отчета', 2, 10, "ReportTextZkh");
        resultHtml += '</div>';

        resultHtml += '<div class="form-group" id="decision" style="display:none">';
        var resolutionHtml = renderFieldBlock('Резолюция', 2, 10, "ReportDecisionZkh");
        ...
        resultHtml += renderFieldBlockNoFormControl('Новый срок', 2, 10, "ReportNewDeadlineZkh");
        resultHtml += '</div>';

        resultHtml += '<div class="form-group" id="changeRequestComment" style="display:none">';
        resultHtml += renderFieldBlock('Комментарий', 2, 10, "ReportResolutionCommentZkh");
        resultHtml += '</div>';
    ...
    function getQuestionData(Id, onsuccess, onfail) {
        SP.SOD.executeOrDelayUntilScriptLoaded(function () {
            ...
            ctx.load(Instance, "Include(IssueMeetingZkh)");
    ...
    function getAssignmentData(Id, onsuccess, onfail) {
        ...
        SP.SOD.executeOrDelayUntilScriptLoaded(function () {
            ...
            ctx.load(assignmentInstance, "Include(AssignmentIssueZkh, AssignmentTextZkh, AssignmentNumberZkh)");
            ctx.executeQueryAsync(function () {
    ...
    function OnPostRender(context) {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.DisplayForm) {
            ...
            renderCore.ifget('decision', function (e) {
                ...
                if (context.ListData.Items[0].ReportDecisionZkh == deadlineTriggeredValue) {
                ...
                if (context.ListData.Items[0].ReportDecisionZkh == changeRequestTriggeredValue) {
                    renderCore.ifget('changeRequestComment', function (e) {
                        $(e).css('display', 'block');
                    });
                }
            ...
            getAssignmentData(context.ListData.Items[0].ReportAssignmentZkh.split(';')[0], function (e) {
                var questionNumber = e.get_item("AssignmentIssueZkh").get_lookupValue();
                var assignmentText = e.get_item("AssignmentTextZkh"); 
                var assignmentNumber = e.get_item("AssignmentNumberZkh");;

                getQuestionData(e.get_item("AssignmentIssueZkh").get_lookupId(), function (e) {
                    var meetingNumber = e.get_item("IssueMeetingZkh").get_lookupValue();
        ...
        $('#' + assignmentLinkId.replace(/(:|\.|\[|\]|\$)/g, "\\$1")).on("change", function (event) {
            if (this.value) {
                getAssignmentData(this.value, function (e) {
                    $('#LinkedAssignmentText').html(e.get_item("AssignmentTextZkh"));
                });
            }
            else {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.NewForm) {
            var hasContext = document.referrer && ~document.referrer.indexOf('Lists/AssignmentZkhList');
            if (hasContext) {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.EditForm) {
            if (context.ListData.Items[0].ReportAssignmentZkh) {
                getAssignmentData(context.ListData.Items[0].ReportAssignmentZkh.split(';')[0], function (e) {
                    $('#LinkedAssignmentText').html(e.get_item("AssignmentTextZkh"));
                });
            }
    ...
    SP.SOD.executeOrDelayUntilScriptLoaded(function () {
        init();
        SP.SOD.executeOrDelayUntilScriptLoaded(function () {
            RegisterModuleInit(SPClientTemplates.Utility.ReplaceUrlTokens("~siteLayouts/15/GS.Zkh.Web/Scripts/csr/renderReportZkh.js"), init);
        }, 'sp.js');
    }, 'clienttemplates.js');
})();
