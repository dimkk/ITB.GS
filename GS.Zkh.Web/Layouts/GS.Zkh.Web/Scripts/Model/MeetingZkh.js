...
(function () {
    ...
    function openParticipantSelectDialog(title, container, isMulti) {
        var options = {
            title: title,
            url: isMulti ? _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/GS.Zkh.Web/Pages/ParticipantsSelection.aspx?Multi=1" : _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/GS.Zkh.Web/Pages/ParticipantsSelection.aspx",
            width: 800,
            height: 400,
    ...
    function openQuestionAttachmentsDialog(title, id) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + (String).format("/_layouts/15/GS.Zkh.Web/Pages/IssueAttachmentZkh.aspx?ID={0}", id),
            width: 800,
            height: 400,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
                // ToDo: 
    ...
    function openOrderRgAddDialog(title) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + "/Lists/OrderZkhList/Unused.aspx",
            width: 1024,
            height: 768,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    function openIssueRgCreateDialog(title) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + "/Lists/IssueZkhList/NewForm2.aspx",
            width: 1024,
            height: 768,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
                SP.UI.ModalDialog.RefreshPage(dialogResult);
    ...
    function Assignment(data) {
        ...
        selfAssign.selectExecutor = function () {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/GS.Zkh.Web/Pages/ExecutorSelection.aspx",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function (dialogResult, returnValue) {
        ...
        selfAssign.selectSoexecutors = function () {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/GS.Zkh.Web/Pages/ParticipantsSelection.aspx?Multi=1",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    function issue(data) {
        ...
        this.calcContent = ko.computed(function () {
            ...
            return str || this.IssueDescriptionZkh();
        }, this);

        // вычислимое поле для списка докладчиков
        this.calcReporters = ko.computed(function () {
            var reporters = [];
            if (this.IssueReporterZkh && this.IssueReporterZkh()) {
                reporters.push(this.IssueReporterZkh().get_lookupValue());
            }

                if (this.IssueCoReportersZkh() != null) {
                    $.each(this.IssueCoReportersZkh(), function () {
        ...
        this.httpLink = ko.observable(
            (String).format("{0}//{1}/sites/gca/Lists/IssueZkhList/DispForm2.aspx?ID={2}&ContentTypeId={3}",
        ...
        this.gotoEditQuestion = function () {
            window.open(_spPageContextInfo.webAbsoluteUrl + '/Lists/IssueZkhList/DispForm2.aspx?ID=' + data.Id, '_blank');
        }
...
/*         this.loadSoreporters = function () {
        ...
        this.showAttachments = function () {
            openQuestionAttachmentsDialog(
                (String).format("Вложения вопроса повестки №{0}", data.IssueNumberZkh), data.Id);
        };

        // диалог отображения объектов
        this.showObjects = function () {
            openQuestionObjectsDialog(
                (String).format("Объекты вопроса повестки №{0}", data.IssueNumberTextZkh), data.Id);
        };
        ...
        //        url: _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/GS.Zkh.Web/Pages/SearchQuestion.aspx",
        //        width: 800,
        //        height: 400,
        //        dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    function meeting(data) {
        ...
        this.changeTitle = ko.computed(function () {
            var number = this.MeetingNumberZkh();
            var nowDate = new Date();
    ...
    function meetingViewModel() {
        ...
        self.loadData = function () {
            self.availableMeetingStatuses(getChoicesFromMetadata(modelMetaData.meeting.fields, "MeetingStatusZkh"));
                    ...
                    try {
                        ...
                        while (enumerator.moveNext()) {
                            ...
                            if (newQuest.IssueCoReportersZkh() != null)
                                $.each(newQuest.IssueCoReportersZkh(),
                        ...
                        while (enumerator.moveNext()) {
                            var docTypeLink = enumerator.get_current().get_item("MeetingAttachmentDocTypeZkh");
                            ...
                            attach = new meetingAttach({
                                AttachmentDescription: enumerator.get_current().get_item("MeetingAttachmentDescriptionZkh"),
                                MeetingLink: enumerator.get_current().get_item("MeetingAttachmentMeetingZkh"),
                                Id: enumerator.get_current().get_item("ID"),
                                FileUrl: fileInfo.FileUrl,
                                FileName: fileInfo.FileName,
                                FilePath: "",
                            ...
                            if (enumerator.get_current().get_item("MeetingAttachmentProtocolCopyZkh")) {
        ...
        self.moveIssue = function (current, isUp) {
            ...
            $.appWebContext.executeQueryAsync(function () {
                ...
                prevListItem.set_item('IssueNumberZkh', currentNumber);
                currentListItem.set_item('IssueNumberZkh', prevNumber);
                prevListItem.update();
        ...
        self.exportToWord = function () {
            __doPostBack('Export.WordByTemplate', JSON.stringify({ ListName: 'MeetingZkhList', ListItemId: $.listItemId }));
        };
