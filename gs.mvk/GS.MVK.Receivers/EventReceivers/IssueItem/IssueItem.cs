    ...
    public class IssueItem : SPItemEventReceiver
    {
        ...
        protected void SetIssuePStatus(SPItemEventProperties properties, StatusEnum status)
        {
            ...
            try
            {
                ...
                {
                    //Устанавливаем статус "Рассмотрено на МВК" только при заполнении решения
            ...
            {
                Log.Unexpected(ex, "Непредвиденная ошибка при установке статуса планового вопроса (ID = {0}) для вопроса МВК (ID = {1})", issuePId, properties.ListItemId);
            }
        }
