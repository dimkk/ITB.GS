...
(function () {
    ...
    function openQuestionObjectsDialog(title, data) {
        ...
        if (data.agendaQuestionObjects().length > 1)
            ...
            url = url + (String).format("/Lists/Object/DispForm.aspx?ID={0}", data.agendaQuestionObjects()[0].IssueObjectIssueIdLand().get_lookupId());
        var options = {
            title: title,
            url: url,
            width: 1024,
...
/*         this.loadSoreporters = function () {
    ...
    function meetingViewModel() {
        ...
        self.removeAgendaQuestion = function (agendaQuestion) {
            var currentNumber = agendaQuestion.IssueNumberLand();
